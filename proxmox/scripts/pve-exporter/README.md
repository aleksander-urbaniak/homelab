# 🖥️ Proxmox PVE Exporter Setup Guide

This guide describes how to set up the Prometheus PVE Exporter on Proxmox VE for monitoring.

## Folder Contents

- [create-pve-exporter-user.sh](create-pve-exporter-user.sh): creates or verifies a read-only Proxmox user and API token for metrics scraping.
- [deploy_proxmox_pve_exporter.sh](deploy_proxmox_pve_exporter.sh): installs `prometheus-pve-exporter`, writes the local config, creates a systemd service, and performs a basic health check.

The instructions below document the same setup manually. Keep real API token values out of git.
