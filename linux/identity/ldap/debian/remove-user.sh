...
# --- CONFIGURATION ---
OLD_USER="exampleuser"
NEW_USER="exampleadmin"
NEW_UID=1000
NEW_GID=1000
NEW_PASS="${NEW_PASS:-}"
...
if [[ -z "$NEW_PASS" ]]; then
    echo "[!!!] NEW_PASS is not set."
    echo "      Run with NEW_PASS='<temporary-password>' bash remove-user.sh"
    echo "      or replace this helper with your own private migration workflow."
    exit 1
fi
